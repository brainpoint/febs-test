  ...
  end_custom(token, type) {
    try {
      ...
      if (map.has(token.id)) {
        ...
        obj.count++;
        obj.delayAvg = febs.utils.bigint_add(obj.delayAvg, delta)
  ...
  report() {
    if (cluster.isMaster) {
      ...
      this.statistics.forEach(function(value, key){
        ...
        if (value.success.count)  value.success.delayAvg = febs.utils.bigint_dividedBy(value.success.delayAvg, value.success.count);
        if (value.failure.count)  value.failure.delayAvg = febs.utils.bigint_dividedBy(value.failure.delayAvg, value.failure.count);

        value.success.delayAvg = febs.utils.bigint_dividedBy(value.success.delayAvg, 1000000);
        value.failure.delayAvg = febs.utils.bigint_dividedBy(value.failure.delayAvg, 1000000);

        ret += `    [${key}]:` + os.EOL;
        ret += `        request          : ${value.request}` + os.EOL;
        ret += `        timeout          : ${timeout}` + os.EOL;
        ret += `        success          : ${value.success.count}` + os.EOL;
        ret += `               delay-avg : ${febs.utils.bigint_toFixed(value.success.delayAvg)} ms` + os.EOL;
        ...
        ret += `               delay-avg : ${febs.utils.bigint_toFixed(value.failure.delayAvg)} ms` + os.EOL;
        ...
        for (var kk in value) {
          if (kk != 'request' && kk != 'success' && kk != 'failure') {
            ...
            if (value[kk].count)  value[kk].delayAvg = febs.utils.bigint_dividedBy(value[kk].delayAvg, value[kk].count);

            value[kk].delayAvg = febs.utils.bigint_dividedBy(value[kk].delayAvg, 1000000);
            value[kk].delayAvg = febs.utils.bigint_dividedBy(value[kk].delayAvg, 1000000);
    
            let sk = `        ${kk}`;
            while (sk.length < 25) sk += ' ';
            ret += sk + `: ${value[kk].count}` + os.EOL;
            ret += `               delay-avg : ${febs.utils.bigint_toFixed(value[kk].delayAvg)} ms` + os.EOL;
