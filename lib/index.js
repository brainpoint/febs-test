...
var testPerformance = require('./testPerformance');
var os = require('os');
...
module.exports = {
  ...
  */
   ...
   */
  testPerformance : (opt)=>{(new testPerformance()).test(opt||{});},
};
...
function start(opt, workCB) {
  opt.processNum = opt.processNum || 50;
  opt.clientNumPerProcess = Math.ceil(opt.clientTotal / opt.processNum);
  ...
  if (cluster.isMaster) {
    ...
    let progressTimer = setInterval(()=>{
      ...
      let info = '...' + Math.floor((opt._currentDurtion/opt.testDurtion)*100) + '%  ' + `free memory: ${Math.floor(os.freemem() / 1024 / 1024)} MB` + `    cpu stage: ${os.loadavg()[0]} %`;
      console.log(info);
      logger.debug(info);
    }, 5000);
